  ...
  async (req: AuthedRequest, res) => {
    ...
    if (nextLevel > maxHints) {
      return res.status(409).json({
        ...
        hintsExhausted: true,
        message:
          "You've used all available hints. You can now request an AI fix patch.",
      });
    }

    if (nextLevel > 3) {
      return res.status(400).json({
        error: "Hint levels only go up to 3",
      });
    }

    const diagnostics = parseDiagnostics(
      attempt.compilerRawJson,
      attempt.submission.problem.language
    );

    const previousHints = attempt.hintUsages.map((h) => h.content);

    // Only record the HintUsage once we have a complete hint.
    // A failed generation must not consume one of the student's hints.
    let content: string;
    ...
    try {
      ...
      return res.status(503).json({
        ...
        message:
          "The AI tutor didn't return a complete hint. Please try again — this didn't use up a hint.",
      });
    }
...
helpRouter.post(
  ...
  async (req: AuthedRequest, res) => {
    ...
    if (attempt.aiPatch) {
      return res.json(attempt.aiPatch);
    }
